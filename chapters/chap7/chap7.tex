...
For reference, we give the $\tau_\text{a}$ for the bulk water and water/vapor interface in the third and fourth column, respectively.
...
The result we obtained from VSFG spectra in Chapter \label{CHAP_SFG} is that the blue shift of the HB band in the Im$\chi^{(2)}$, 
...
Similar to the \CSHB, which we have defined in Eq.\thinspace\ref{eq:C_s_HB}, for the H-bonds in instantaneous interfaces, 
...
To expain the $C_2(t)$ for the \LiN solution/vapor and LiI/vapor interfaces, 
...
For comparison, we also fit the C2(t) function of the water molecules in each interface in Table 1 with a single exponential. 
...
\paragraph{Alkali-nitrate solution}
We calculated the anisotropy decay of water molecules in \Li and nitrate ions' hydration shells in the LiNO$_3$ solution/air interface. 
...
For the NaNO$_3$ solution/air interface, the average values of the $C_2(t)$ are also shown in 
...
Finally we also did the same calculation of $C_2(t)$ for the KNO$_3$ solution/air interface. 
...
%The fitted parameters of anisotropy decay of water molecules in ion-shell in alkali nitrate solution/air interfaces and the water/vapor interface. 
...
%[DO THE SAME ANALYSIS FOR THE LINO3 SOLUTION?]
\paragraph{Alkali nitrate solution/vapor interface}
...
To check the conclusion, we calculate the average number of hydrogen bonds per water molecule. For the same sets of water molecules, which is belongs to three types of water molecules respectively, we obtained the results shown in Fig.\thinspace\ref{fig:nhb_per_wat_lino3_256w}. 
...
It is evident that $C_2(t)$ for DDAA and DD$'$AA molecules do not decay exponentially (Fig.\thinspace\ref{fig:2LiI-124w_c2_fit_biexp_7wat_2ps_class} 
...
Finally, D$'$AA molecules' the inertial-librational motion can not be seen (Fig.\thinspace\ref{fig:2LiI-124w_c2_fit_biexp_7wat_2ps_class}). 
...
%    \caption{
%[NOT CLEAR!]
%(A) Time dependence of $C_2(t)$ for water molecules in different HB environments at the LiI solution/vapor interface.
%(B) Time dependence of the $C_2(t)$ for DD$'$A, DD$'$AA, and D$'$AA water molecules at the LiI solution/vapor interface.
%}%
