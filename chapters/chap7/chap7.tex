...
We have combined the IHB and MS methods and determined the reaction rate constants of the H-bonds on the water-vapor interface. 
...
%During the simulation, the temperature of the system is 
%300 K and the volume is a constant, and periodic boundary conditions are used.
For the interface of the NaNO$_3$ solution, the size of periodic box is $(a,b,c)=(15.6530, 15.6530, 31.3060)$ (\AA). % NaNO3, at T = 300 K
For the interface of the KNO$_3$ solution, the size of periodic box is $(a,b,c)=(15.7160, 15.7160, 31.4320)$ (\AA). % KNO3
...
Here, $L=15.7787$, 15.6530, and 15.7160 for \LiN, NaNO$_3$, and KNO$_3$ solution, respectively.
...
The water/vapor interface is modeled with a slab 
made of 121 water molecules in a simulation box of size $15.60 \times 15.60 \times 31.00$ \A$^3$. 
For this water/vapor interface, the DFTMD simulation at 330 K is also calculated.
...
For the DFTMD trajectory of a solution interface, we can now get instantaneous surface (see Fig.\thinspace\ref{fig:distance_ions2surf_lino3_trimed}).
...
\caption{\label{fig:hbacf_C_sh2_2p}The \CHB of water--water hydrogen bonds in the solvation shell 
  ...
  The dashed line is for the interface (thickness $d = 8$ \A) of the LiI solution.}  
\end{figure}
...
For the water/vapor interface and the aqueous solution interfaces, 
...
As shown in Fig.\thinspace\ref{fig:vdos_LiNO3-256w_w_near_nitrate} in Chapter\thinspace\ref{CHAPTER_SFG_Calculation}, 
...
\centering
\caption{ 
    The Dynamical properties of I$^-$--water and nitrate--water hydrogen bonds. The values are expressed in ps.} 
\begin{tabular}{cccc}
\label{tab:properties_anion-water_hbs}
  & Criterion & I$^-$--water & NO$_3^-$--water \\
\hline
...
\label{eq:C_k_HB}.
\end{eqnarray}
When not considering specific ions, we denote $C^\text{(k,X)}_{\text{HB}}(t)$ as $C^{(k)}_\text{HB}(t)$ for short.
%
...
\label{eq:k_k_HB}.
\end{eqnarray}
Using these correlation functions, we can determine the reaction rate constants of breaking and reforming and the lifetimes of solvation shell H-bonding.
...
We calculated the HB dynamics of the H-bonds between water molecules in the first solvation shell and other molecules outside the sphere. 
...
and Fig.\thinspace\ref{fig:shb_c_ln_bk_Shell_pbc}, we conclude that: In the bulk system, although the HB between nitrate and water molecule is significantly 
...
Also, there are still some differences in the HB kinetics outside the dissolving shell of different ions, 
...
The $\ln S^\text{(k)}_\text{HB}(t)$ for H-bonds in the solvation shells of \Li and \I ions (in the water/vapor interface)  based on water-water 
pair HB population operator $h^\text{(k)}(t)$, as computed from the (a) ADH and (b) AHD criteria of H-bonds.} 
...
%\end{figure}
%
\section{Rotational anisotropy decay of water at the air-water interface}\label{RAD}
...
The anisotropy decay of OH chromophores in water molecules in 0.4 M LiNO3 solution/vapor interface is shown in Fig.\thinspace\ref{fig:c2_LiNO3_inset}.  
...
\paragraph{Alkali-iodine solutions}
The anisotropy decay $C_2(t)$ for the whole LiI interface of aqueous solution is given in Fig.\thinspace\ref{fig:c2_2LiI_16_inset}.
This function decays faster than that of air-water interface, indicating that H-bonds
at the interfaces of alkali-iodine solutions reorient faster than in the air-water interface. 
%
\begin{figure}[h]
\centering
\includegraphics [width=0.36\textwidth] {./diagrams/c2_2LiI_16_inset} 
\setlength{\abovecaptionskip}{0pt}
  \caption{\label{fig:c2_2LiI_16_inset}Time dependence of $C_2(t)$ of OH bonds at the interfaces of 0.9 M LiI solution 
and of pure water (dashed line) at 330 K.} 
\end{figure}
%
...
The average results of $C_2(t)$ is shown in Fig.\thinspace\ref{fig:C2_ln_itp_pbc}.
...
Moreover, the interface of the KNO$_3$ solution, we also did the same calculation of $C_2(t)$. 
...
(for the cases of LiNO$_3$ and NaNO$_3$ solutions, respectively) or accelerate (for the case of KNO$_3$ solution), 
