...
it is directly related to the orientational correlation function $C_2(t)$\cite{Tokmakoff1996,Rezus2006,Schmidt2007,Yagasaki2009,Bakker2010}
\begin{equation}
R(t)=\frac{2}{5}C_2(t).
\label{eq:tcf2}
\end{equation}
The orientational correlation function $C_2(t)$ is given by the rotational time-correlation function 
\begin{equation}
...
In the simulations, we concentrate on water and we consider a unit vector that is 
...
\begin{equation}
C_2(t)=Ae^{-t/\tau_2},
\label{eq:c2_for_differet_thicknesses}
\end{equation}
...
\end{figure} 
We see that when the interface thickness is relatively small, $\tau_2$ is also relatively small.
...
It can be seen that if we start from the instantaneous interface, we can reach a consistent conclusion on the issue of estimating the thickness of the water/vapor interface, 
...
1. be more quantitative, providing the rate constants from the exp fit; 

2. Check the code again, and to calculate the C2(t) for LiNO3 solution again to make sure the result is correct.).  
...
at the interfaces of alkali-iodine solutions reorient faster than at the water/vapor interface. 
%
\begin{figure}[h]
\centering
...
\setlength{\abovecaptionskip}{0pt}
  \caption{\label{fig:c2_2KI_2NaI_2LiI_16} (TODO: add legend for neat water) Time dependence of $C_2(t)$ for OH bonds in water molecules at the interface of the alkali-iodine solutions and of neat water (dashed line).
% at 330 K; 0.9 M
}
...
(TODO: Here I should comment on which is the physical meaning of a double exponential.)
...
(TO CHECK: What do I mean for "surface" and "bulk"?  instantaneous surface, or 2\AA surface?)}
...
(TODO: Here is on the surface or not?).
...
In this paragraph, we will answer the question whether the water molecules in the first solvation hydration shell of different ions have different orientation dynamics. 
...
\setlength{\abovecaptionskip}{0pt}
\caption{\label{fig:C2_ln-nn-kn_itp_pbc}The $C_2(t)$ for water in the solvation shell of \li, \na, K$^+$ and nitrate ions in the interface system of alkali-nitrate solutions. 
...
(These values come from the radial distribution functions $g_{\text{NN-OW}}(r)$, $g_{\text{Na-OW}}(r)$ and $g_{\text{OW-OW}}(r)$,
for bulk alkali nitrate solution, as shown in Fig.\thinspace\ref{fig:gdr_127_XNO3}.) 
