...
We performed the simulation on a system containing one \Li ion, one nitrate ion 
...
Here, we find that the cations \Li and \Na does not alter the H-bonding network outside the first hydration shell of cations. 
...
Similar to the $C^\text{s}_\text{HB}(t)$ for the H-bonds in instantaneous interfaces, for a given $r_\text{shell}$,
...
Like $C^\text{s}_\text{HB}(t)$, $C^\text{k}_\text{HB}(t)$ show that the HB dynamics is accelerated to a certain extent. 
...
\caption{\label{fig:c2_LiNO3_inset} Anisotropy decay of OH bonds in water molecules in LiNO$_3$ solution/vapor interface.}
\end{figure} 
...
at the interfaces of alkali-iodine solutions reorient faster than in the air-water interface of the neat water. 
%
\begin{figure}[h]
\centering
...
In this paragraph, we try to verify whether the water molecules in the first solvation hydration shell of different ions have different orientation dynamics. 
...
For the interface of the NaNO$_3$ solution, the average value of the $C_2(t)$ functions are shown in 
...
From the above calculation of $C_2(t)$, it can be found that in the interfacial systems of alkali metal nitrate solution, 
...
%[RIGHT?]
Slower rotational anisotropy decay exists for water molecules  at the water/vapor interface of the alkali-iodide solutions, 
