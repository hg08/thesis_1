...
There are geometric\cite{Kumar2007} and energetic criteria\cite{Sciortino1989} to define a HB.
Here we use the geometric one:
Two water molecules are H-bonded if their inter-oxygen distance between of specific tagged pair of water molecules 
...
In this thesis, we use $r^{\text{c}}_{\text{OO}}=3.5$ \AA both for the ADH and the AHD criteria, $\phi^{\text{c}}=30^{\circ}$ for the ADH criterion, 
...
\begin{figure}[H]
\centering
\includegraphics [width=0.36\textwidth] {./diagrams/pure_bk_c}
\setlength{\abovecaptionskip}{0pt}
\caption{\label{fig:pure_bk_c}
The \CHB for bulk water, 
as computed from the ADH (solid line) and the AHD (dashed line) criterion of H-bonds. The donar-accepter interchange process is included.} 
...
a pair of water molecules can form 4 different forms of H-bonds. 
In other words, there are exchange processes, i.e., interchange tunneling and bifurcation tunneling, during the dynamics.
After an exchange processe, a pair of boned molecules still form H-bonds, 
we say that an old HB is broken and a new HB is formed.
In the above definition, we have included this interexchange process. 
In the later paragraphs of this thesis, for the sake of simplicity, 
sometimes we can ignore this interchange process and simply regard the H-bonds before and after the exchange process as the same HB.
The calculated results of the simplified correlation functions (still denoted by \CHB) of water molecules in bulk water 
is shown in Fig.\thinspace\ref{fig:128w_bk_2delta_t_60ps_water_pair_c_ns40}.
%Fig.\thinspace\ref{fig:128w_bk_2delta_t_60ps_water_pair_c_ns40} shows the correlation function \CHB for bulk water over time. 
Comparing Fig.\thinspace\ref{fig:pure_bk_c} and Fig.\thinspace\ref{fig:128w_bk_2delta_t_60ps_water_pair_c_ns40}, 
we found that there are some differences:
because hydrogen exchange is considered in the O--H pair-based HB population in the former, but not in the water--water pair-based HB population in the latter.
...
we plotted \CHB for bulk water, using the ADH and the AHD criteria of HB. We found that the difference from different choice of HB population is not affected by the HB criteria. 
In the following, we will use the simplified HB population based on molecular pairs, i.e., water-water molecule pairs, unless otherwise specified.
\begin{figure}[H]
...
The $c(t)$ for bulk water, 
as computed from the ADH (solid line) and the AHD (dashed line) criterion of H-bonds. The donar-accepter interchange process is neglected.} %Ref:\cite{Khaliullin2013} 
\end{figure}
%
...
and due to detailed balance condition, we obtain 
\begin{eqnarray}
...
Generalised Gradient Approximation of the exchange-correlation functional, BLYP XC functional\cite{Becke1988,LeeC1988}, 
and dispersion force correction, DFT-D3\cite{Grimme10,Klimes2012} have been employed.
...
The correlation functions \CHB from the trajectory of a simulation calculated according to formula \ref{eq:C_HB} 
with the ADH (solid line) and the AHD (dashed line) definition of H-bonds are 
shown in Fig.\thinspace\ref{fig:pure_bk_c_n_k} a. 
The reactive flux $k(t)$ calculated according to formula \ref{eq:k} (see Fig.\thinspace\ref{fig:pure_bk_c_n_k} b) is consistent with the result in \cite{AL96b}.
...
Here we consider the whole system which simulates the water/vapor interface as a whole.
...
In the water/vapor interface, $k(t)$ quickly changes from its initial value in transient period ($\sim 0.2$ ps). 
Beyond this transient period, $k(t)$ decays to zero monotonically (see Fig.\thinspace\ref{fig:pure_bk_and_itp_k}). 
...
whether we take the definition of the ADH or the AHD.
...
Time dependence of $n(t)$ for bulk water and the water/vapor interface from (a) the ADH and (b) the AHD criteria.} 
\end{figure}
...
The function $c(t)$ is regarded as a $P$-dimensional column vector composed by $(c(1),c(2),\cdots,c(P))^T$, and denoted as ${\bf c}$,
...
To determine the instantaneous interface of the system, we adopted their method based on spatial density.
...
At any time point $t$, the volume between the two surfaces 
...
\begin{align}
   ...
   \end{array} \right.
\end{align}
From this definition, we know that $h^{(\text{s})}(t)$ depends on the value of the thickness $d$, therefore,
...
Similar to the correlation function \CHB in Eq. \ref{eq:C_HB}, which describes the fluctuation of general H-bonds,
...
\label{eq:k_s_HB}.
\end{eqnarray}
Using these correlation functions, we can determine the reaction rate constant of breaking and reforming and the lifetimes of interfacial H-bonding.
...
\end{figure}
For the water/vapor interface, we used two geometric criteria of H-bonds to calculate the \hbos and therefore correlation function \CSHB from Eq.\thinspace\ref{eq:C_s_HB}. 
...
This feature means that as $d$ increases, the interface's HB dynamics will converge and eventually tend to bulk water's HB dynamics.
...
Specifically, to determine the water molecules staying in the interface layer, 
we sample at regular intervals, and then calculate the correlation function \CHB for these water molecules in the interface layer and then their a statistical average.
As the thickness $d$ changes, the \CHB for the interface will change. 
Figure \ref{fig:128w_itp_pure_water_pair_c_ihb_scheme1} shows how the function \CHB depends on the thickness $d$.
The panel (a) and (b) use HB definition criterion ADH, and AHD, respectively.
Comparing Fig.\thinspace\ref{fig:128w_itp_pure_water_pair_c_ihb} and Fig.\thinspace\ref{fig:128w_itp_pure_water_pair_c_ihb_scheme1}, we found that
...
as computed from (a) the ADH and (b) the AHD criteria. 
These results are based on the IMS every 4 ps.} 
...
\FloatBarrier
\paragraph{Depth-dependence of $k$ and $k'$} 
To find the reaction rate constants $k$ and $k'$, we also have two methods: 
...
correlation functions \CHB, $n(t)$ and $k(t)$ of the H-bonds of these identified water molecules.
...
We see that, whether it is $k$, $k'$ or $\tau_\text{HB}$, their changing \emph{trend} with the thickness of the interface is only slightly affected by the calculation methods. 
...
when $d$ is large enough ($d > d_0 \sim 4$ \AA), these two constants agree well quantitatively. 
...
and (b) the HB lifetime $\tau_\text{HB}$ on the interface thickness $d$, obtained by the IHB and the IMS, respectively.
The corresponding $k$, $k'$ and $\tau_\text{HB}$ in bulk water are also drawn with dashed lines as a reference.
...
We also found that when we focus on the molecules in the interface layer with $d < d_0$, 
the values of the reaction rate constants does not depend on the method we use. 
That is, the $k$ obtained by the IHB method is relatively larger than by the IMS and $k'$ is smaller. 
Since $\tau_\text{HB} = 1/k$, large $k$ directly leads to a relatively shorter HB lifetime. 
...
we found that when the interface thickness $d$ is larger than $d_0$, 
no matter which statistical method is used, the value of the calculated reaction rate constants of the interface water is \emph{greater} than that in bulk water. 
Therefore, since the HB lifetime can be calculated by the relation $\tau_\text{HB} = 1/k$, the value of $\tau_\text{HB}$ 
for water molecules at the water/vapor interface is smaller than that for bulk water.

Furthermore, we have found from Fig.\thinspace\ref{fig:128w_itp_pure_water_pair_k_k_prime_ihb_both_schemes} that as the interface thickness $d$ increases, 
...
can get as good results as the IMS method for $d>d_0$. 
Because the IHB method is more concise to operate, it can be used to calculate the HB dynamics and thus HB lifetime for the water/vapor interface 
when $d$ is larger than $d_0$. For the water/vapor interface, $d_0$ is approximately $4$ \A \ or equals to the size of $\sim$2 water molecules. This result coincides with the previous theoretical results based on MD simulation\cite{Townsend1985,Taylor1996,Morita2000} and VSFG spectroscopy\cite{Stiopkin2011,Tyrode2013},
which have shown that isotropic properties are already present
...
\caption{\label{tab:k_k_prime_tau_128w_pure_ihb_scheme1_ADH} 
    The $k$ and $k'$ for the interfacial HB dynamics of the water/vapor interface through the IMS, with the ADH criteria.} 
\begin{tabular}{cccc}
 $d$ (\AA) & $k$ (ps$^{-1}$) & $k'$ (ps$^{-1}$) & $\tau_{\text{HB}} (=1/k)$ (ps) \\
\hline
...
\centering
\caption{\label{tab:k_k_prime_tau_128w_pure_ihb_AHD} 
    The $k$ and $k'$ for the interfacial HB dynamics of the water/vapor interface through the IMS, with the AHD criteria.} 
\begin{tabular}{cccc}
 $d$ (\AA) & $k$ (ps$^{-1}$) & $k'$ (ps$^{-1}$) & $\tau_{\text{HB}} (=1/k)$ (ps) \\
\hline
...
\section{Summary}
In this chapter, we have reviewed the interfacial HB population operator, 
and correlation functions based on these operators. 
Then the HB dynamics calculations for bulk water and the water/vapor interface are based on these correlation functions. 
...
Based on the DFTMD simulations, starting from the functions \CHB, $n(t)$, and $k(t)$, 
...
For the water/vapor interface, we have studied the HB dynamics for instantaneous interfaces using the combination of two different statistical methods, 
the IMS and the newly introduced IHB.
We have found that as the thickness of the interface layer increases, 
the HB reaction rate constants tend to the ones in bulk water.
From the results for the water/vapor interface, we conclude that from the perspective of HB dynamics,
the thickness of the water/vapor interface is about 3 \A. This value is smaller than that obtained from the VSFG spectra 
(Ref. Paragraph \thinspace\ref{sfg_ln}) for the \LiN/vapor interface. 
This result is understandable, because of the presence of ions in solutions affects the HB network. 
We will analyze these effects in the next chapter.
