...
There are temporal, geometric\cite{Kumar2007} and energetic criteria\cite{Sciortino1989} to define a HB.
...
\setlength{\abovecaptionskip}{0pt}
\caption{\label{fig:128w_c_itp_bk_ns40}Time dependence of \CHB for bulk water.} 
%\caption{\label{fig:128w_c_itp_bk_ns40}Time dependence of \CHB for the bulk water at 300 K with density $\rho =1.00$ g/cm$^3$.} 
...
Besides $h(t)$, here we also discuss the definition of another possible definition of the HB population operator ${h}'(t)$.
...
initial set of H-bonds\cite{DC87,Starr2000}.
...
previous works\cite{AL96,Kessler2015}. 
...
is always greater than that for bulk water, whether we take the definition of ADH or AHD.
...
Therefore, $k$ and $k'$ are determined from the matrix ${\bf A} = \begin{bmatrix} {\bf c} & {\bf n} \end{bmatrix}$, i.e., 
\begin{equation}
...
After the surface (instantaneous interface) is defined, we can define an instantaneous interface layer for any non-uniform fluid system. 
...
Here, we use $d$ to denote the thickness of the interface. 
...
By investigating the characteristics of HB dynamics for these layers, we can obtain the dynamical characteristics of various solution/vapor interfaces. 
...
This feature means that as $d$ increases, the interface's HB dynamics will converge and eventually tend to bulk water's hydrogen bond dynamics.
...
For comparison, we also calculate the HB dynamics of water molecules at the interface obtained by sampling the interfacial molecules 
(for details, see Appendix \ref{ihb_and_selection} for details). 
...
when we use the method of interfacial molecule sampling, the dependence of the correlation function \CHB
...
functions \CHB, $n(t)$, $k(t)$ (\CSHB, $n^\text{(s)}(t)$, $k^\text{(s)}(t)$), and the rate constants $k$, $k'$.
\begin{figure}[H]
\centering                                         
...
as computed from (a) the ADH and (b) the AHD criteria. 
These results are based on interfacial molecule sampling every 4 ps.} 
...
To find the reaction rate constants $k$ and $k'$, we also have two choices. 
We can see the correlation functions \CSHB, $n^\text{(s)}(t)$ and $k^\text{(s)}(t)$;
or, we can first identify the water molecules at the instantaneous interface at each time point $t$, and start from the corresponding 
...
and the lifetime $\tau_\text{HB}$ obtained by the two different methods, i.e., the instantaneous Interfacial Hydrogen Bond (IHB) and interfacial molecule sampling. 
...
for the HB dynamics of the interface did not produce much difference.
\begin{figure}[H]
\centering
...
and (b) the HB lifetime $\tau_\text{HB}$ on the interface thickness $d$, obtained by the IHB and interfacial molecule sampling, respectively.
...
We also found that when we focus on the molecules in the interface layer whose thickness is less than $d_0$, 
the values of the reaction rate constants does not depend on the method we use. 
That is, the $k$ obtained by the IHB method is relatively larger than by the interfacial molecule sampling and $k'$ is smaller. 
Since $\tau_\text{HB} = 1/k$, large $k$ directly leads to a relatively shorter HB lifetime. 
This result is related to our definition of IHB, and it is the same as our expectation: 
The definition of interfacial H-bonds (or \hbos) makes the HB break rate 
on the interface artificially increased. At the same time, the interfacial molecule sampling retains the original rate constant of H-bonds, 
but it may include the contribution of bulk water molecules to the rate constant. 
That is why the interfacial molecule sampling slightly underestimate the $k$. 
...
we found that when the interface thickness $d$ is larger then $d_0$, 
...
can get as good results as the method of interfacial molecule sampling for $d>d_0$. 
Because the method of IHB is more concise to operate, it can be used to calculate the HB dynamics and thus HB lifetime for the water/vapor interface 
...
we can approximate the interfacial HB dynamics, by either the IHB and the interfacial molecule sampling method if the thickness of the interface is large enough, i.e., $d>d_0$.
...
\caption{\label{tab:k_k_prime_tau_128w_pure_ihb_scheme1_ADH} 
    The $k$ and $k'$ for the interfacial HB dynamics of the water/vapor interface through the interfacial molecule sampling, with ADH criteria.} 
\begin{tabular}{cccc}
 $d$ (\AA) & $k$ (ps$^{-1}$) & $k'$ (ps$^{-1}$) & $\tau_{\text{HB}} (=1/k)$ (ps) \\
\hline
...
\centering
\caption{\label{tab:k_k_prime_tau_128w_pure_ihb_AHD} 
    The $k$ and $k'$ for the interfacial HB dynamics of the water/vapor interface through the interfacial molecule sampling, with AHD criteria.} 
\begin{tabular}{cccc}
 $d$ (\AA) & $k$ (ps$^{-1}$) & $k'$ (ps$^{-1}$) & $\tau_{\text{HB}} (=1/k)$ (ps) \\
\hline
...
the interfacial molecule sampling and the newly introduced IHB.
We have found that as the thickness of the interface layer increases, 
the HB reaction rate constants tend to the ones in the bulk water.
