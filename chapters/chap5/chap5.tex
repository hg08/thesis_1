...
Similar to the correlation function \CHB in Eq.\thinspace\ref{eq:C_HB}, which describes the fluctuation of general H-bonds,
...
as computed from the (a) ADH and (b) AHD criteria. \CSHB is calculated from the IHB method.} 
\end{figure}
...
Moreover, regardless of the ADH or AHD definition of a HB, this conclusion is basically valid.
...
\CHB is calculated from the IMS method, and the sampling rate is 1/4 per ps.} 
...
This result shows that the two different statistical methods (see Appendix \ref{ihb_and_selection}) 
