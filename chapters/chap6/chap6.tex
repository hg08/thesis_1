...
The \SHB for the DFTMD simulated interfacial and bulk water is shown in Fig.\thinspace\ref{fig:128w_s_itp_bk_ns40}.
%-------------------
\begin{figure}[hbtp]
\centering
...
The calculation results obtained from the DFTMD simulations are in good agreement with numerous experimental results and simulation results. \cite{Tongraar2006} 
...
Similar to the correlation function $C_\text{HB}(t)$ in Eq. \ref{eq:C_HB}, which describes the fluctuation of the general H-bonds,
 we can define the correlation function $C^s_\text{HB}(t)$ that describes the fluctuation of the interface H-bonds: 
\begin{eqnarray}
...
Figure \ref{fig:128w_itp_pure_water_pair_c_ihb_scheme1} shows how the function $C_\text{HB}(t)$ changes with the thickness of the interface.
The sub-figure (a) and (b) use HB definition cirterion 1, and 2, respectively.
Comparing Fig.\thinspace\ref{fig:128w_itp_pure_water_pair_c_ihb} and Fig.\thinspace\ref{fig:128w_itp_pure_water_pair_c_ihb_scheme1}, we can see that
when we use the method of selecting molecules in the interface to calculate, the dependence of the correlation function $C_\text{HB}(t)$ ($C^s_\text{HB}(t)$) 
...
functions $C_\text{HB}(t)$ ($C^s_\text{HB}(t)$), $n_\text{HB}(t)$, $k(t)$, and the rate constants $k$, $k'$ determined by these correlation functions.
...
We can see that, whether it is $k$, $k'$ or $\tau_\text{HB}$, their changing trend with the thickness of the 
...
\end{figure}
We can see from Fig.\thinspace\ref{fig:128w_itp_pure_water_pair_k_k_prime_ihb_both_schemes} that no matter which method is used, 
...
In Fig.\thinspace\ref{fig:128w_itp_pure_water_pair_k_k_prime_ihb_both_schemes}, the $k$, $k'$ and $\tau_\text{HB}$ for the \emph{bulk} water are also drawn with dashed lines as a reference.
...
From Fig.\thinspace\ref{fig:128w_itp_pure_water_pair_k_k_prime_ihb_both_schemes}, we can also find that as the interface thickness $d$ increases, 
...
can get as good results as the method of molecule selection  when the thickness is large enough. 
...
when the $d$ is large enough (in this case, $d \sim 3$ \A or the size of 2--3 water molecules).
...
and the new method does not require such sampling. Therefore, the new method is more efficient than the traditional method. 
