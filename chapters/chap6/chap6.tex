...
%Each water molecule has two H atoms and one O atom. Therefore, 
...
To verify this point of view, we performed a DFTMD simulation of the bulk water system with a total time of 60 ps, 
and used the two different HB definitions (ADH and AHD) to calculate $k(t)$. 
...
% about the pure water interface, the aqueous solution containing ions, and the HB dynamics in the interface corresponding to the aqueous solution.
...
of water--water H-bonds for water/vapor \emph{interface}, calculated from the trajectory of a DFTMD simulation.
The definition of $h(t)$ is based on specific H--O bond, instead of water-water pairs.}
\end{figure}
...
which had been used in the study of HB dynamics of liquid water. \cite{AL96,Khaliullin2013}
The $k(t)$ calculated from the trajectory of water molecules in DFTMD simulations, is reported in Fig.\thinspace\ref{fig:121}. 
...
It can be seen from Fig.\thinspace\ref{fig:128w_bk_itp_50ps_n_from_k_in_with_2_hb_def_type2} that the $n(t)$ of the water/vapor interface
is always greater than the value of $n(t)$ in the bulk water. 
%This means that the HB between a pair of water molecules at time $t$ is broken
%and the distance between them is less than $r^c_\text{OO}=3.5$ \AA at the water/vapor interface is more likely to occur than in bulk water. 
...
\setlength{\abovecaptionskip}{0pt}
  \caption{\label{fig:121}Time dependence of the $k(t)$ for the interface of neat water, calculated by DFTMD simulations.
  The inset shows the log-log plot of the $k(t)$.}
\end{figure}
\begin{figure}[H]
\centering
...
Time dependence of the population functions $n(t)$ for bulk water and the water/vapor interface from (a) ADH (b) AHD criteria.} 
\end{figure}
...
Fig.\thinspace\ref{fig:128w_bk_itp_50ps_n_from_k_in_with_2_hb_def_type2} a and b
show the function $n(t)$ in Eq.\thinspace\ref{eq:n_from_k_in} for bulk water and water/vapor interface, respectively. 
...
Comparing the two figures \ref{fig:128w_bk_itp_50ps_n_from_k_in_with_2_hb_def_type2} a and b,
we find that the maximum value of $n(t)$ at the water/vapor interface is slightly higher than that in bulk water. 
...
\hline
  ...
  ADH & 0.296  & 0.988 & 3.380  & 0.323 & 0.765 & 3.101 \\
  AHD & 0.288 & 1.149 & 3.470 & 0.314 & 0.887 & 3.184 \\
\end{tabular}
\end{table}
%
\begin{table}[htb]
...
\hline
  ...
  ADH & 0.115  & 0.039 & 8.718 & 0.157  & 0.068 & 6.372\\
  AHD & 0.105  & 0.047 & 9.496 & 0.155  & 0.088 & 6.472 \\
\end{tabular}
\end{table}
% 
...
The definition of  $h^{s}[{\mathbf r}(t)]$ is critical to help us to efficiently obtain the H-bonds' dynamic characteristics in an
instantaneous interface system of any thickness. %Note that the definition of HB here is based on water molecule pairs or O-H pairs. 
...
We will discuss the dependence of the correlation functions \CHB, $C^s_\text{HB}(t)$, and the reaction rates $k$ and $k'$ on the interface thickness $d$ in the next two paragraphs.
%
\FloatBarrier
...
The $C^s_\text{HB}(t)$ for the instantaneous interfacial H-bonds with different thickness ($d$), based on water-water 
pair HB population operator $h^{s}(t)$, as computed from the (a) ADH and (b) AHD criteria of H-bonds.} 
\end{figure}
For the pure water interface, we used two geometric criteria of H-bonds to calculate the $h^s(t)$ and therefore correlation function $C^s_\text{HB}(t)$. 
...
As the thickness $d$ changes, the $C_\text{HB}(t)$ in the interface will also change. 
Figure \ref{fig:128w_itp_pure_water_pair_c_ihb_scheme1} shows how the function $C_\text{HB}(t)$ changes with the thickness $d$.
The sub-figures a and b use HB definition criterion ADH, and AHD, respectively.
...
when we use the method of Molecule Selection (MS) in the interface, the dependence of the correlation function $C_\text{HB}(t)$  
on the interface thickness is very consistent with that of $C^s_{HB}(t)$ for large $d$. 
Moreover, regardless of the AHD definition or the ADH definition of the HB, this conclusion is basically valid. 
Beside the correlation functions $C_\text{HB}(t)$ or $C^s_\text{HB}(t)$ in the interface, we will further examine the correlation 
...
The $C_\text{HB}(t)$ for the instantaneous interfacial H-bonds with different thickness $d$, based on water-water pair HB population operator $h(t)$, 
...
To find the reaction rate constants $k$ and $k'$, we also have two choices. we can start from the correlation functions $C^s_\text{HB}(t)$, $n^s(t)$ and $k^s(t)$;
...
The definition of interfacial H-bonds (or $h^s(t)$) makes the HB break rate 
...
Finally, one point is worth adding. Because the more realistic HB dynamical properties of interface molecules are between the results of the above two methods. 
It is possible to approximate the true appearance of the interfacial HB dynamics, 
by both the IHB and the MS methods if the thickness of the interface is selected large enough. 
...
the thickness of the air-liquid interface of water is about 4 \A. This value is smaller than that obtained from the SFG spectra 
(Ref. paragraph \thinspace\ref{sfg_lino3_interface}), and this result may have reference significance for our study of the influence of ions on the H-bonds 
outside the solvation shell of ions. 
These results inspire us: the presence of ions in the solution affects the HB network. We will analyze these effects in the next chapter.
%
\begin{table}[H]%[htb]
\centering
