...
The time dependence of (a) $C_\text{HB}(t)$ and (b) ln$S_\text{HB}(t)$ of all water--water and nitrate--water H-bonds at 300 K
...
We need the parameter $d$ to define the thickness of the instantaneous interface.
Now we can write the interface HB population operator $h^{s}[{\mathbf r}(t)]$ as follows:
It has a value 1 when the particular tagged molecular pair are bonded \emph{and} both molecules are inside the instantaneous interface,
and zero otherwise.

Similar to the correlation function $C_\text{HB}(t)$ in Eq. \ref{eq:C_HB}, which describes the fluctuation of the general H-bonds,
we define the correlation function $C^s_\text{HB}(t)$ that describes the fluctuation of the interface H-bonds: 
\begin{eqnarray}
C^s_{\text{HB}}(t)=\langle h^s(0)h^s(t) \rangle/\langle h^s\rangle
\label{eq:C_s_HB}.
\end{eqnarray}
%
Based on the function $C^s_\text{HB}(t)$, we can define a similar correlation functions $k^s(t)$, $n^s_\text{HB}(t)$, $S^s_\text{HB}(t)$.
Therefore, using these new correlation functions, we can study the dynamics of interface H-bonding.
%
...
The calculation results are shown in Fig.\thinspace\ref{fig:128w_itp_pure_water_pair_c_ihb}.
We find that the greater the thickness of the interface is selected, 
the slower the relaxation of the interface H-bonds. When the thickness of the interface is greater than a certain thickness $d^c$ ( $\sim$ 5 \AA),
...
The correlation functions $C_\text{HB}(t)$ for the instantaneous interfacial H-bonds, based on water-water pair HB population operator $h(t)$, 
...
The characteristic of this algorithm is to first select the molecules in the interface at each moment and make a statistical
...
the correlation function $C_\text{HB}(t)$ for the water molecules located in the interface and do a statistical average.
As the thickness $d$ of the instantaneous interface changes, the auto-correlation function of H-bonds in the interface we get will also change. 
Figure \ref{fig:128w_itp_pure_water_pair_c_ihb_scheme1} shows how the function $C_\text{HB}(t)$ changes with the thickness $d$ of the interface.
...
when we use the method of selecting molecules in the interface, the dependence of the correlation function $C_\text{HB}(t)$ ($C^s_\text{HB}(t)$) 
...
not just the correlation functions $C_\text{HB}(t)$ or $C^s_\text{HB}(t)$ in the interface, we will further examine the correlation 
functions $C_\text{HB}(t)$ ($C^s_\text{HB}(t)$), $n(t)$, $k(t)$, and the rate constants $k$, $k'$ determined by these correlation functions.
...
interface is only slightly affected by the calculation method we use. In other words,
no matter which method is used, the changes of $k$ and $k'$ with respect to $d$ is the same. 
...
The dependence of (a) the rate constants $k$ and $k'$ and (b) the lifetime $\tau_\text{HB}$ on the interface thickness,
...
In sub-figure (a), the $k$ of bulk water is represented by a \emph{black dashed} line, and its $k'$ is represented by a \emph{blue dashed} line;
...
\end{figure}
Moreover, we can see from Fig.\thinspace\ref{fig:128w_itp_pure_water_pair_k_k_prime_ihb_both_schemes} that 
when the thickness is large enough ($d_0 \sim 3$ \AA), these two constants agree well quantitatively. 
This very good result shows that the two extreme statistical methods (see Appendix \ref{ihb_and_selection}) 
used to calculate the HB dynamics of the interface did not produce much difference for the time scale (10$^2$ ps) 
and the scale ( 10$^2$ \AA ) of the simulation box we currently use.

We also found that when we look at the molecules in the interface whose thickness is less than $d_0$, the values of the rate constants depend on the method we use. 
...
At the same time, since $\tau_\text{HB} = 1/k$, this directly leads to a relatively shorter HB lifetime using the IHB method. 
...
From Fig.\thinspace\ref{fig:128w_itp_pure_water_pair_k_k_prime_ihb_both_schemes}, we also find that as the interface thickness $d$ increases, 
...
At the same time, because the IHB is easier to operate, this method can calculate the HB dynamics and thus HB lifetime on the interface 
...
and the new method does not require such sampling. Therefore, the IHB method is more efficient than the traditional method. 
...
The distances between ions and the instantaneous surfaces (blue meshes) for a slab of aqueous LiNO$_3$ solution.}
\end{figure}
...
which indicates that nitrate ions are on the instantanous interface. 
...
minimum of the X--oxygen RDF, i.e., $R_\text{XO}^c=4.1$, 3.7 \A, for X = I$^-$ and nitrate O. We have still used $30^{\circ}$ for the anglar cutoff.\cite{Chowdhuri2006}
\begin{figure}[htbp]
\centering
...
The results of the $C_\text{HB}(t)$ are shown in Fig.\thinspace\ref{fig:X-O_c_lii_xlogscale}. 
%\begin{figure}[htbp]
%\centering
