...
For both HB definitions, we find that the decay of $C_\text{HB}(t)$ and $S_\text{HB}(t)$ for N--W hydrogen bonds is much faster 
...
Time dependence of (a) $C_\text{HB}(t)$ and (b) ln$S_\text{HB}(t)$ of all water--water (W--W) and nitrate--water (N--W) bonds
...
Assumimg that the $z$-axis is the normal direction, the distances between the particle and the instantaneous surfaces are:
%
\begin{eqnarray}
...
We find that, the \I ion resides near the surface, while alkali metal ions does not have such a strong tendency toward the surface. 
\begin{figure}[H]%
    \centering
...
%/home/gang/Github/hb_in_interface_IHB/result/128w_itp_wat_pair_hbacf_h_ihb_1.dat
To investigate ions' effects, we calculated the HB population correlation functions 
in ions' slovation shells at aqueous electrolyte interfaces, and we compared it to that of the pure water/vapor interface obtained in the previous chapter (see paragraph XXX).
...
at interfaces of alkali-iodine solutions: LiI, NaI and KI. 
For reference, we also give the $\tau_\text{a}$ for bulk water and the water/vapor interface in the third and fourth column, respectively.
The continuum HB lifetime $\tau_{\text{a}}$ in the 
solvation shell of alkali metal (iodine) ions is larger (smaller) than 
that of H-bonds at the water/vapor interface. 
For the LiI solution, the water molecules bound to the cation ion
...
Besides, we find that $\tau_{\text{a}}$ for the I$^-$-shell is smaller than that for water/vapor interface in general.
%
...
The \emph{ratio} of the number of iodide ions to the number of other (WHY OTHER?) water molecules 
...
\setlength{\abovecaptionskip}{0pt}
  ...
  (a) LiI solution/vapor interface; (b) NaI solution/vapor interface.
...
which proves that the N-W hydrogen bonds is weaker than W-W ones. 
The result we obtained from VSFG spectra in paragraph 5.1 is that the blue shift of the HB band in the Im$\chi^{(2)}$, 
...
we also calculate the survival probability for interfaces with different sizes of thickness.[HAVE TO CHANGE] 
...
We calculated the HB dynamics for the H-bonds between water molecules in the second solvation shell. 
...
The $\ln S^\text{(k)}_\text{HB}(t)$ for H-bonds in the second solvation shells of \Li and \I ions (in the water/vapor interface) 
...
For the LiN solution/vapor interface, the distribution $P(n)$ is shown in Fig.\thinspace\ref{fig:distribution_nhb_ln} a--d.
...
The red (black) line is for the \LiN solution/vapor (water/vapor) interface.}
\end{figure}
...
(1) For thinner layers (Fig.\thinspace\thinspace\ref{fig:distribution_nhb_ln} a--c, i.e., $d\le 3$ \AA), $P(n=0)$ for \LiN solution/vapor interface is smaller than that for the water/vapor interface.
This result implies that there are less water dangling OH bonds at the \LiN solution/vapor interface, 
compared to the water/vapor interface.
(2) The $P(n=1)$ for \LiN/vapor interface is larger for $d$ is thin enough. This means that the water at the topmost interface layer are more H-bonded than those at the 
water/vapor interface.
(3) In particular, for $d=2$ \AA, we have $P(n=2)$ for \LiN is larger than that for the water/vapor interface, which is approximately zero.
...
(4) From the distribution $P(n)$, we again obtain the consistent result, i.e., when $d>3$ \AA, the difference between \LiN/vapor interface and water/vapor interface disappears.
...
anisotropy decays and SFG spectroscopy.
...
i.e., water molecules at the LiI solution/vapor interface have larger reorientation relaxation rate than that at the water/vapor interface.
...
\setlength{\abovecaptionskip}{0pt}
  ...
  % we have to consider the entire slab to see the effect of LiI/solution interface, but LiI interface has large I propensity for surface.
...
\end{figure} 
%
The distribution of the number of HBs per OH bond for the LiI solution/vapor interface is also calculated and analyzed. 
For the LiI solution/vapor interface, the distribution $P(n)$ is shown in Fig.\thinspace\ref{fig:distribution_nhb_lii} a--d.
%
...
for the LiI solution/vapor interface is smaller than that for the water/vapor interface.
...
From the probability distribution of ions for the LiI
solution/vapor interface, see Fig. 6.3, which shows that the I$^-$ has more propensity for surface than the Li$^+$. 
We conclude that there are more O--H$\cdots$I bonds at the LiI solution/vapor interface than in bulk phase.
(4) We can find that $P(n=0)$ of the LiI solution/vapor interface is larger than that of the water/vapor interface, 
this implies that at the LiI solution/vapor interface, the O--H$\cdots$I 
...
The red (black) line is for the LiI solution/vapor (water/vapor) interface.}
\end{figure}
%
...
two clear structural properties of alkali iodide solution/vapor interface.
...
We calculated the anisotropy decay of water molecules in ions' hydration shells in alkali-nitrate (alkali-iodide) solution/vapor interfaces. 
...
These values are obtained from the position of the first minimun of the RDFs $g_{\text{NN-OW}}(r)$ (NN: nitrate nitrogen), $g_{\text{Li-OW}}(r)$ and $g_{\text{OW-OW}}(r)$,
...
For the NaNO$_3$ solution/vapor interface, the average values of the $C_2(t)$ are also shown in 
...
Finally we also calculated $C_2(t)$ for the KNO$_3$ solution/vapor interface. 
...
For the dynamic process in a short time, we find a relation between the reorientation relaxation time and the radius of hydration shell.
...
We also find that $\tau_2$ of the water molecules in the hydration shell decreases with the increase of the radius of the solvation shell 
...
To expain the $C_2(t)$ for the \LiN and the LiI solution/vapor interfaces, 
...
We find that there is a correlaton between reorientation and the strength of the H-bonds:
...
The peaks' position (unit: \A) of RDFs for the LiI solution/vapor, the \LiN solution/vapor and the water/vapor interfaces (see Table \ref{fig:gdr_127_LiNO3} and \ref{fig:gdr_124_2LiI}). 
}
\begin{tabular}{cccc}
...
However, the reorientation relaxation time $\tau_2$ of the water molecules in the [WHICH?] solvation shell 
