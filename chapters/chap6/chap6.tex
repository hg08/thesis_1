...
All simulations in this chapter were performed at the DFT (BLYP + D) level and at 300 K within the canonical ensemble.
...
For both HB definitions, we find that the decay of $C_\text{HB}(t)$ and $S_\text{HB}(t)$ for N--W bonds is much faster 
than that for W--W bonds. From the relation \ref{eq:calculate_hb_lifetime_from_s}, the faster relaxation of $S_\text{HB}(t)$ implies that 
the N--W bonds have shorter lifetime than the W--W bonds in bulk phase. 
The calculation results are in good agreement with numerous experimental and simulation results\cite{PS03,Vrbka2004,Tongraar2006,Otten2007}. 
It means that nitrate has structure-breaking ability, or that the N--W interaction is weaker, compared with W--W interaction.
...
Time dependence of (a) $C_\text{HB}(t)$ and (b) ln$S_\text{HB}(t)$ of all W--W and N--W bonds
for the slab of LiNO$_3$ solution, as computed from the ADH (solid line) and the AHD (dashed line) criterion of H-bonds. 
...
The W--W bonds represents the H-bonds between all pairs of water molecules in the entire slab.
} 
\end{figure}
%
...
Assuming that the $z$-axis is the normal direction, the distances between the particle and the instantaneous surfaces are:
%
\begin{eqnarray}
...
The result for the HB dynamics are in agreement with what we have learn in Chapter\thinspace\ref{CHAPTER_SFG}  
...
(2) The vibration frequency of water molecules from the \LiN/vapor interface is higher than that at the water/vapor interface (see Fig.\thinspace\ref{fig:surf_x-vs-l_x_d1-5}).
...
%
\subsection{Alkaline iodide solutions}
...
Figure \ref{fig:prob_dist_Li_surf_I_surf}--\ref{fig:prob_dist_K_surf_I_surf} shows the density of the distance between the anions (cations) 
...
\begin{figure}[H]%
    ...
    \caption{
Density distribution of ions at the LiI/vapor and the NaI/vapor interfaces.   
(A) 
...
(B)
Density distribution of the K$^+$--surface and the I$^-$--surface distances at the KI/vapor interface, respectively. 
%The horizontal axis represents the distance between the ion and the instantaneous surface, which is defined in 
%Eq.\thinspace\ref{eq:distance_particle2surf_1}. 
%The \emph{distance} refers specifically to $d_{\text{X},1}$, the distance between the ion 
...
The average of the continuum HB lifetimes $\langle\tau_{\text{a}}\rangle=\int_0^\infty S_\text{HB}(t) dt$ (unit: ps) in the first solvation shell of I$^-$ ion 
...
%/home/gang/Github/hb_in_interface_IHB/result/128w_itp_wat_pair_hbacf_h_ihb_1.dat
To investigate ions' effects, we calculated the HB population correlation functions for the H-bonds between 
ions' first and second solvation shells at aqueous electrolyte interfaces,
We call them the H-bonds \emph{in the solvation shell}.
Then we compared these correlation functions to that of the pure water/vapor interface obtained in the previous chapter 
(see Paragraph\thinspace\ref{PARA_IHB}). 
...
Table\thinspace\ref{tab:tau_hb_alkali_iodine} lists the continuum HB lifetime of H-bonds between the first and the second solvation shell 
of I$^-$ ion and of alkali metal ion, respectively, at interfaces of alkali-iodine solutions: LiI, NaI and KI. 
For reference, we also give the $\tau_\text{a}$ for bulk water and the water/vapor interface in Table\thinspace\ref{tab:tau_hb_alkali_iodine}.
The continuum HB lifetime $\tau_{\text{a}}$ for the H-bonds in the 
solvation shell of alkali metal (iodine) ions is larger (smaller) than 
that at the water/vapor interface. 
For the LiI solution, the water molecules bound to \Li, on average, have a continuum HB lifetime $\tau_{\text{a}} \sim 0.24$ ps,
which is longer than that of molecules bound to \I and at the water/vapor interface. 
Besides, we find that $\tau_{\text{a}}$ for the I$^-$-shell is smaller than that at the water/vapor interface in general.
%
...
\setlength{\abovecaptionskip}{0pt}
\caption{\label{fig:hbacf_C_sh2_2p}The \CHB of W--W bonds in the solvation shell 
  of (a) cations and (b) I$^-$ at the interfaces of 0.9 M LiI, NaI and KI solutions, respectively.
  The solid and dashed line is for the instantaneous 1-\AA water/vapor interface (see Paragraph\thinspace\ref{PARA_IHB}) and bulk water, respectively.
...
Figure \thinspace\ref{fig:hbacf_C_sh2_2p} a and b show that the \CHB of H-bonds within the alkali cations' and \I's solvation shell decay faster 
...
From Fig.\thinspace\ref{fig:hbacf_C_sh2_2p} b, we found that for all three alkali-iodine solutions, the \CHB for solvation shell water molecules 
...
and the relaxation time $\tau_{\text{R}}$ for each of them, which can be determined according to formula \ref{eq:tau_relaxation}. 
...
Water molecules below the surface are surrounded by more water molecule neighbors than molecules at the interface.
Since the upper half of the surface is vacuum and the cations here do not aggregate on the surface. 
Therefore, \rI > \rcation, 
so the influence of the cation on the HB relaxation is not as strong as the influence of the \I ion on the surface. 
...
\setlength{\abovecaptionskip}{0pt}
  \caption{\label{fig:124_2LiI-2NaI_hbacf_S} Time dependence of the \SHB: 
...
of water molecules at solution/vapor interfaces, obtained from \emph{ab inito} simulations. 
...
which proves that the N-W bonds is weaker than W-W ones. 
...
which is also consistent with the smaller relaxation time given by the correlation functions \CHB and \SHB.
...
Similar analyse for N--W bonds is also done by Yadav, Choudhary and Chandra by first-principles MD simulations\cite{Yadav2017}. 
...
The $\ln{S_\text{HB}(t)}$ for the W--W bonds and N--W bonds at the \LiN/vapor interface is shown in 
Fig.\thinspace\ref{fig:256_LiNO3_hbacf_sh_no3}. 
The $\ln{S_{\text{HB}}(t)}$ shows that
the N--W bonds are weaker than W--W bonds, i.e., nitrate ions accelerate the HB dynamics in water.
%
...
\caption{\label{fig:256_LiNO3_hbacf_sh_no3} The \SHB of W--W and N--W bonds at the 
  \LiN/vapor interface. The inset is the plot of ln\SHB. 
...
is also analyzed in terms of the survival probability $S_{\text{HB}}(t)$\cite{AKS86,JT90,AL96}, 
...
\setlength{\abovecaptionskip}{0pt}
  \caption{\label{fig:X-O_c_lii_xlogscale}Time dependence of the intermittent correlation functions \CHB of I$^-$--water (I$^-$--W) and W--W bonds. 
...
\setlength{\abovecaptionskip}{0pt}
  \caption{\label{fig:wat-wat_s_lii}Time dependence of the continuous correlation functions \SHB of I$^-$--water (I$^-$--W) and W--W bonds.}
\end{figure} % 300 K
For the anion-oxygen (X--O) bonds, we can use a similar criteria. The cutoff values for X--oxygen distance are obtained from the positions of the first
...
the results of the continuous correlation functions for both definitions (the ADH and the AHD criteria) for the H-bonds are shown in Fig.\thinspace\ref{fig:wat-wat_s_lii} 
for I$^-$--water bonds. The results of W--W bonds are also included for comparison in both Fig.\thinspace\ref{fig:X-O_c_lii_xlogscale} 
and \thinspace\ref{fig:wat-wat_s_lii}.
For both the ADH and the AHD definitions of H-bonds, it is found that the I$^-$--water bonds show faster dynamics than W--W bonds 
...
No matter from the ADH or the AHD criteria, the average lifetime $\langle\tau_\text{a}\rangle$ of I$^-$--water bonds is shorter than that of NO$_3$--water bonds.
...
we can draw the following conclusion: I$^-$--water bonds and NO$_3^-$--water bonds are both weaker than W--W bonds. In particular, 
the I$^-$--water bonds is slightly weaker than the NO$_3^-$--water ones.
\begin{table}[htbp]
\centering
\caption{ 
    The dynamical properties of I$^-$--water and N--W bonds within the ADH (AHD) criterion.} 
\begin{tabular}{ccc}
...
We will extend the IHB dynamics in Paragraph \ref{PARA_IHB} to H-bonds around ions in aqueous solutions. 
...
Below we will combine the interface defined by the first and the second solvation shell of ions, 
and Luzar-Chandler's HB population \cite{AL96} to calculate the HB
dynamics for the H-bonds between the first and the second solvation shell of ions.
...
We use the parameter $r_\text{shell}$ to denote the radius of the first solvation shell.
...
for a given $r_\text{shell}$, we define the correlation function $C^\text{(k,X)}_\text{HB}(t)$ that describes the fluctuation of \emph{the solvation shell H-bonds} for ion X: 
\begin{eqnarray}
...
%
\paragraph{Alkaline nitrate solutions}
\begin{figure}[H]
\centering
...
the HB population $h^\text{(k)}(t)$, as computed from (a) the ADH and (b) the AHD criteria.
...
We calculated the HB dynamics for the solvation shell H-bonds. 
The choice of the shell radius $r_\text{shell}$ comes from the RDFs (see Fig.\ref{fig:gdr_127_XNO3}). 
Figure \ref{fig:c_and_s_ln_bk_pbc} a shows that the N--W bonds, or H-bonds in the first solvation shell of nitrate,
is significantly weaker than the W--W bonds.
...
then we could conclude that the H-bonds in the second solvation shell of nitrate will be weaker than 
...
Specifically, Fig.\thinspace\ref{fig:shb_c_ln_bk_Shell_pbc} shows that the relaxation process of H-bonds 
between the water molecules in the second solvation shell of the nitrate ion is \emph{not} faster than the relaxation process of H-bonds between the water molecules in the second solvation shell of the \li ion. 
In general, this result implies that the HB strength between water molecules in the ions' second slovation shells are not affected by the nature of the ions evidently.
...
The probability distributions of ions in the solution/vapor interface of the LiI and NaI solutions with respect to the depth 
...
\begin{figure}[h]
\centering
\includegraphics [width=0.64\textwidth] {./diagrams/shb_c_lii_bk_new_Shell_pbc_r5}
\setlength{\abovecaptionskip}{0pt}
...
as computed from (a) the ADH and (b) the AHD criteria. 
The \CHB (dashed line) for bulk water is also plotted in panel (a) and (b) respectively.}
...
The $\ln S^\text{(k)}_\text{HB}(t)$ for H-bonds in the second solvation shells of \Li and \I ions (at the water/vapor interface) 
as computed from (a) the ADH and (b) the AHD criteria of H-bonds.} 
...
\paragraph{Water/vapor interface} For the water/vapor interface, using the method introduced in Paragraph \ref{para:II}, 
...
Similar to the correlations \CHB and \SHB for interfacial water molecules, as the interface thickness increases, the $\tau_2$ converges to a fixed value,
...
The anisotropy decay of OH bonds in water molecules at 0.4 M LiNO3/vapor interface is shown in Fig.\thinspace\ref{fig:c2_ln_itp_pbc_surf1_ave_2A}.  
...
As we known in Paragraph\thinspace\ref{sfg_ln}, the LiNO$_3$ was inserted at one of the two interfaces, 
...
%The water molecules considered are in instantaneous layer with $d=6$ \AA below the surface. 
%}
%\end{figure} 
\begin{figure}[H]
\centering
...
First, we calculated the distribution of the number ($n$) of H-bonds formed by a OH group at a given instantaneous interface by
\begin{equation}
...
For the LiN/vapor interface, the distribution $P(n)$ is shown in Fig.\thinspace\ref{fig:distribution_nhb_ln} a--d.
...
\setlength{\abovecaptionskip}{10pt}
\caption{\label{fig:distribution_nhb_ln} Distribution $P(n)$ of the number of H-bonds formed by a OH group at instantaneous layers with thickness $d$ ($d=1,\cdots,4$ \AA). 
...
\setlength{\abovecaptionskip}{10pt}
\caption{\label{fig:distribution_nhb_ln_s1} Distribution $P(n)$ of the number of H-bonds formed by a OH group at instantaneous layers with thickness $d$ ($d=1,\cdots,6$ \AA). 
The red (black) line is for the interfaces (bulk) phase.}
\end{figure}
From Fig.\thinspace\ref{fig:distribution_nhb_ln} and \ref{fig:distribution_nhb_ln_s1}, we find:
(1) For thinner layers (Fig.\thinspace\thinspace\ref{fig:distribution_nhb_ln} a--c, i.e., $d\le 3$ \AA), 
$P(n=0)$ for \LiN/vapor interface is smaller than that for the water/vapor interface.
...
(2) The $P(n=1)$ for the \LiN/vapor interface is larger for $d$ is thin enough. This means that the water at the topmost interface layer are more H-bonded than those at the 
...
(4) From the distribution $P(n)$, we again obtain the consistent result, i.e., when $d>3$ \AA, the difference between the \LiN/vapor and the water/vapor interface disappears.
Both distributions converge to $P(n)$ for bulk water.

These results demonstrate that the structural properties in the instantaneous layer with $d\le 3$ \AA is the main source of interfacial properties, such as HB lifetime, 
anisotropy decays and VSFG spectroscopy.
...
For the LiI/vapor interface, the distribution $P(n)$ is shown in Fig.\thinspace\ref{fig:distribution_nhb_lii} a--d.
%
(1) From both Fig.\thinspace\ref{fig:distribution_nhb_lii} and Fig.\thinspace\ref{fig:distribution_nhb_lii_s1}, we find that the ratio of water dangling OH bonds 
for the LiI/vapor interface is smaller than that for the water/vapor interface.
(2) When $d> 3$ \AA the distribution $P(n)$ does not change anymore.
...
From the probability distribution of ions for the LiI/vapor interface, see Fig.\thinspace\ref{fig:prob_dist_Li_surf_I_surf} a,
which shows that the I$^-$ has more propensity for surface than the Li$^+$. 
We conclude that there are more O--H$\cdots$I bonds at the LiI/vapor interface than in bulk phase.
(4) We can find that $P(n=0)$ of the LiI/vapor interface is larger than that of the water/vapor interface, 
this implies that at the LiI/vapor interface, the O--H$\cdots$I 
bonds replace the role of H-bonds at these layers.
%
...
\setlength{\abovecaptionskip}{10pt}
\caption{\label{fig:distribution_nhb_lii} Distribution $P(n)$ of the number of H-bonds formed by a OH group at instantaneous layers with thickness $d$ ($d=1,\cdots,4$ \AA). 
...
\setlength{\abovecaptionskip}{10pt}
\caption{\label{fig:distribution_nhb_lii_s1} Distribution $P(n)$ of the number of H-bonds formed by a OH group at instantaneous layers with thickness $d$ ($d=1,\cdots,6$ \AA). 
The red (black) line is for the interfaces (bulk) phase.}
\end{figure}

Therefore, the increasing density of the O--H$\cdots$I bonds, and decreasing density of water dangling OH bonds are 
two clear structural properties of alkali iodide/vapor interface.
The decreasing of water dangling OH bonds can explain reduction of the intensity of the 3700 \cm peak in the alkaline iodide solutions, 
and the increasing  density of the O--H$\cdots$I bonds can explain the blue-shift of the 3200--3600 \cm band.

\FloatBarrier
\section{Rotational anisotropy decay of water molecules in ions' solvation shells}\label{RAD_SHELL}
In this paragraph, we answer the question whether the water molecules in the first solvation shell of different ions have different orientation dynamics. 
...
\setlength{\abovecaptionskip}{0pt}
\caption{\label{fig:C2_ln-nn-kn_itp_pbc}The $C_2(t)$ for water molecules in the solvation shell of \Li, \Na, \K, \I and nitrate ions 
at the alkali-nitrate (alkali-iodide) solution/vapor interfaces. 
For comparison, the $C_2(t)$ for bulk water (black line) is also shown.}
\end{figure} % 300 K
...
(Also in this case ,these values are obtained from the RDFs $g_{\text{NN-OW}}(r)$, $g_{\text{Na-OW}}(r)$ and $g_{\text{OW-OW}}(r)$,
...
The N--W and W--W bonds have very close local structure, while the iodide-water (I$^-$--W) bonds are of significantly different local structure.
...
\section{Summary}
We have investigated the effect of alkali-nitrate and alkali-iodide ions on the HB dynamics
of water molecules at solution/vapor interfaces.
%我们视气液界面中的所有水分子作为一个整体，来计算其氢键动力学。涉及到离子溶解壳层内的氢键动力学时，我们通过在采样时间点水分子是否处于离子溶解壳内来确定需要考虑的水分子。
Nitrate--water HB dynamics decay faster than water--water's, 
which proves that the N--W bonds is weaker than W--W bonds. 
...
The instantaneous interfaces are used in the analyse. The main result is that the ratio of the free OH bonds for the water/vapor interface is an important factor 
...
and the faster anisotropy decay of water molecules at the interface is the effects of free OH stretch and Hydrogen--Iodide (H--I) bond.
