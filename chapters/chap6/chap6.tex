...
After the instantaneous surface (interface) is defined, we can define an interface layer for any non-uniform fluid system. 
...
To find the reaction rate constants $k$ and $k'$, we also have two choices. we start from the correlation functions \CSHB, $n^\text{(s)}(t)$ and $k^\text{(s)}(t)$;
...
when the $d>d_0$. In the case of water/vapor interface, $d_0 \sim 4$ \A \ or the size of 2--3 water molecules. This result coincides with the previous theoretical results based on MD simulation\cite{Townsend1985,Taylor1996,Morita2000} and VSFG spectroscopy\cite{Stiopkin2011,Tyrode2013},
