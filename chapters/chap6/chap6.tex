...
Besides $h(t)$, here we also discuss the definition of another possible definition of the HB population operator $\tilde{h}(t)$.
...
The calculated results of the correlation functions (still denoted by \CHB) of $\tilde{h}(t)$ of water molecules in bulk water 
...
For bulk water, we can compare the results of the method in this paragraph with the results of 
...
Using these quantities we discussed the change in the HB dynamics at the interface with the interface layer's thickness.
...
Specifically, to determine which water molecules staying in the interface layer, 
we sample at regular intervals, and then calculate the correlation function \CHB for the water molecules in the interface layer and their a statistical average.
...
when the thickness $d$ is large enough ($d_0 \sim 4$ \AA), these two constants agree well quantitatively. 
...
\section{Summary}
In this chapter, we have introduced the HB population operators, interfacial HB population operators, 
and various correlation functions \CHB, \SHB, $n(t)$, and $k(t)$ based on this operator. 
...
We have studied the HB dynamics for instantaneous interfaces using two different statistical methods, the new IHB and MS.
