...
%[DONE: For which system? is the same system?]
...
\caption{\label{fig:gdr_127_XNO3}
	RDFs for the alkali nitrate solutions (Appendix \ref{SOLUTION_VAPOR_PROPERTIES}).
}
\end{figure} % 300 K
\begin{figure}[H] 
\centering
...
%If the $r_\text{shell}$ is chosen as the radius of the first solvation shell of ions, the correlation functions $c^{(k)}(t)$ fluctuations too much, and we can not see the correlation
...
As we have known in Paragraph\thinspace\ref{sfg_ln}, the LiNO$_3$ was inserted at one of the two interfaces, 
