...
from which we can obtain the interface's nonlinear polarizability to 
...
Both the measured and calculated VSFG spectra shows a reduced intensity in the lower frequency portion region,
...
we determined the instantaneous interface with Willard and Chandler's method based on spatial density, 
and proposed a statistical scheme based on the interfacial HB (IHB) population for the instantaneous interface.
Combining this method with interfacial molecule sampling, we can get more realistic HB dynamics for the water/vapor interface.
The IHB method is also extended to general interface systems, such as the solvation shells of ions in aqueous solutions.
%
The difference between the HB dynamics in the first solvation shell of the \Li and that of nitrate-water Hydrogen (H-) bonds
...
Through solvation shell HB (SHB) dynamics, we obtained the HB dynamics for the H-bonds between water molecules in ion's 
first and second solvation shell. Similarly, we calculated the reorientation dynamics ($C_2(t)$) of the water molecules in the first solvation shells of ions. 
...
In contrast, the HB dynamics for the H-bonds between water molecules in the first and the second solvation shell are not sensitive to the ions' nature.

Finally, the distribution of the number of H-bonds of a OH group at a given instantaneous interface help us to understand the difference between the alkali nitrate
...
molecules and the $\Im \chi^{(2)}$ spectrum of the alkali nitrate interface.
\end{abstract}
