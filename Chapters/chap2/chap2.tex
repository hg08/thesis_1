...
Sch\"{o}dinger equation) is the Density Functional Theory (DFT). [\cite{HK64,KS65}]
...
in place of the more complex $3N_{\text{e}}$-dimensional many-body wave-function.
...
is an explicit orbital functional. However, it is much more appropriate to
...
In the applications in this thesis, we use the BOMD method, as implemented in CP2K package. [\cite{Vandevondele2005}] 
In the BOMD  
...
\section{Calculation of VSFG Spectroscopy for Water/Vapor Interfaces}
...
\begin{figure}[h]                                                                                                                                   
\centering
\includegraphics[width=0.4\textwidth]{./diagrams/sfg_1a}
\setlength{\abovecaptionskip}{0pt}
\caption{The schematic of the VSFG process which involves IR and Raman transitions. The dashed line denotes a virtual level in the Raman transition.}\label{fig:sfg_1a}
\end{figure}
...
In Eq.\thinspace(\ref{eq:chi_R}) $\rho=e^{-\beta H/Z}$ for a system with Hamiltonian $H$ and partition function $Z$ at reciprocal temperature $\beta=1/kT$;
...
$\left\langle \dot{\bf{A}}_{\eta\xi}(t) \dot{\bf{M}}_{\kappa}(0)\right\rangle$ in Eq.\thinspace(\ref{eq:chi}), because $v_z(t)$
